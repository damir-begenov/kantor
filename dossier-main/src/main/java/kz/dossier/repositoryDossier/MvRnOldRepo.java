...
public interface MvRnOldRepo extends JpaRepository<MvRnOld, Long> {
    ...
    @Query(value= "select * from imp_rn.mv_rn where (cadastral_number = ?1 and address_rus = ?2) or (cadastral_number = ?1 and address_kaz = ?2) ORDER BY register_reg_date asc", nativeQuery = true)
